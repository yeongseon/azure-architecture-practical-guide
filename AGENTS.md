...
1. **Launch the user's Chrome with a dedicated debug profile and a remote-debugging port.** A dedicated `--user-data-dir` avoids Chrome's block on debugging the default profile, and OS-level Platform SSO / Company Portal still satisfies device compliance. The command below is a **macOS example**; on Windows or Linux substitute the Chrome binary path and use an OS-appropriate `--user-data-dir`:
    ```bash
    # macOS example — adjust the binary path and --user-data-dir on Windows/Linux
...
3. **Verify the port is bound before attaching:** `curl --silent http://localhost:9222/json/version`, and poll `http://localhost:9222/json` to detect when the target blade URL has loaded.
...
Common failure: relaunching the Chrome binary while Chrome is already running just opens a tab in the existing (non-debug) process and silently ignores `--remote-debugging-port`. Always confirm the port responds (for example with `curl --silent http://localhost:9222/json/version`) before assuming the debug instance is up.
...
6. **Documentation Quality Gate**: Run `python scripts/validate_doc_quality.py --all` to scan documentation for structural and safety defects that the other gates do not cover: missing or mis-ordered tail sections (`## See Also` before `## Sources`), missing required section headings for Best Practices / Operations / Troubleshooting templates, Azure CLI code blocks lacking a nearby explanation table or using short flags (`-g`, `-n`), Mermaid blocks missing `diagram-id` metadata, generated placeholder phrases, thin lab sections, broken repository-tracked file references, and real-looking sensitive values (subscription IDs, Application Insights keys, secrets, unmasked GUIDs). In CI the `Validate Documentation Quality` job (in `quality-gates.yml`) blocks on **changed Markdown files only** (`--changed-only`, so historical debt never becomes a permanent failure) and runs a full-repo advisory `--all` scan for visibility. This validator is a **shared core** rule set vendored across the Azure Practical Guide series, not a blind verbatim copy: the core logic is identical in every repository, and each repo adjusts only the `Per-repository profile` block at the top of the script (`REQUIRE_CONTENT_VALIDATION`, `SERVICE_LIVE_HOST_RE`, `REPO_DIRS`). For this repository the profile sets `REQUIRE_CONTENT_VALIDATION = False` — consistent with the section above, this repo deliberately does not enforce per-file `content_validation` frontmatter — defines no service-specific live-host rule, and sets `REPO_DIRS` to this repo's tracked source directories (`scripts`, `labs`, `infra`, `src`, `docs`) so local file references outside them are treated as reader-created and never false-blocked. This gate is a Prevention item of the cross-repo quality-gate audit ([azure-container-apps-practical-guide#385](https://github.com/yeongseon/azure-container-apps-practical-guide/issues/385)); it is vendored per repository so each guide carries its own script and workflow.

7. **Visual Content Gate (advisory)**: Run `python scripts/validate_visual_content.py --all` to report factual-claim pages carrying **no visual aid** (no Mermaid fence, no `shot()` capture reference, no Markdown image). Scope is self-contained in this repo's copy of the script (this repository has no `scripts/lib/content_scope.py`): the factual sections are `platform/`, `waf/`, `patterns/`, `workload-guides/`, `operations/`, and `architecture-reviews/`, minus their navigation-only index pages; design labs and the practical journey follow the series lab/evidence model and stay out of scope, as do `reference/` and `start-here/`. The gate is **WARN-only** — per-page warnings plus a coverage summary, always exit 0 — making visual coverage a tracked CI metric without blocking on historical debt. In CI the `Validate Visual Content (advisory)` job runs the gate doctests and the full-repo report. Escalation to a blocking gate is a per-repo decision tracked in [azure-container-apps-practical-guide#391](https://github.com/yeongseon/azure-container-apps-practical-guide/issues/391); vendored per repository like the other shared-core gates.
