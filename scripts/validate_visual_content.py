...
- **Scope**: self-contained in this repo's copy (no
  ``scripts/lib/content_scope.py`` here): the factual sections are
  ``platform/``, ``waf/``, ``patterns/``, ``workload-guides/``,
  ``operations/``, and ``architecture-reviews/`` minus navigation-only
  index pages; design labs, the practical journey, reference, and
  start-here follow the lab/lookup/orientation exemptions.
...
def is_in_scope(rel_path) -> bool:
    """Scope policy for this repository (self-contained variant).

    This repo has no ``scripts/lib/content_scope.py``; factual sections are
    its platform / WAF / patterns / workload-guides / operations /
    architecture-reviews trees minus navigation-only index pages. Design
    labs and the practical journey follow the series lab/evidence model;
    reference (incl. generated dashboards) and start-here are lookup or
    orientation content.

    >>> is_in_scope("platform/landing-zones-basics.md")
    True
    >>> is_in_scope("waf/reliability.md")
    True
    >>> is_in_scope("workload-guides/public-web-api/baseline.md")
    True
    >>> is_in_scope("platform/index.md")  # navigation
    False
    >>> is_in_scope("design-labs/lab-01-public-web-baseline.md")  # lab model
    False
    >>> is_in_scope("practical-journey/stage-01-mvp.md")  # staged lab
    False
    >>> is_in_scope("reference/glossary.md")
    False
    >>> is_in_scope("start-here/overview.md")
    False
    """
    rp = str(rel_path)
    top = rp.split("/", 1)[0]
    return top in SCANNED_SECTIONS and rp not in NAVIGATION_INDEXES
