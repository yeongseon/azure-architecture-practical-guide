---
description: Series-wide contributor baseline for Azure Practical Guide language guides — canonical file tree, heading skeletons, navigation patterns, and review checks.
content_sources:
  references:
    - type: self-generated
...
# Language Guide Baseline

This document defines the canonical file tree, heading skeletons, navigation conventions, and quality gates that language guides across the Azure Practical Guide series can share. Each service repo chooses a reference language and then applies this baseline with service-specific content.

## Why This Baseline Exists

Multiple language-guide implementations (Python, Node.js, Java, .NET) reuse the same tutorial structure, recipe categories, and reference pages. A shared baseline prevents structural drift and helps readers move between sibling repos without re-learning the navigation.
