...
Governance and topology choices define the control envelope within which every workload operates. Architecture quality depends on making those controls predictable, scalable, and exception-aware. [Validated]
...
- Keep mandatory controls centralized. [Validated]
- Make exception handling explicit and time-bound. [Observed]
...
2. Can the network topology scale with new subscriptions, regions, and connectivity requirements? [Measured]
3. Is centralized DNS and firewall ownership backed by operational SLAs? [Validated]
...
- Stronger policy inheritance reduces drift but increases the need for transparent exceptions. [Validated]
- Centralized network services lower duplication while increasing shared dependency concentration. [Measured]
...
- Is hub or Virtual WAN design sized for expected regional growth? [Measured]
- Do shared firewall and DNS services have operational ownership and support paths? [Validated]
...
- Exception volume indicates the governance model no longer fits real workload needs. [Measured]

## Decision takeaway

Governance and network topology should provide a stable enterprise control plane, not a permanent source of architectural friction. [Validated]

## Related decisions

- Validate whether network centralization is still aligned to regional growth and M&A scenarios. [Measured]
- Periodically review policy density so teams can still reason about inherited controls. [Observed]
